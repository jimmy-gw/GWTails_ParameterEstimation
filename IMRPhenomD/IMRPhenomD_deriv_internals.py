...
from IMRPhenomD.IMRPhenomD_internals import chiPN,FinalSpin0815,fringdown,fmaxCalc,ComputeIMRPhenDPhaseConnectionCoefficients
from IMRPhenomD.IMRPhenomD_internals import DPhiInsAnsatzInt,PhiInsAnsatzInt,DPhiIntAnsatz,PhiIntAnsatz,DPhiMRD,PhiMRDAnsatzInt
from IMRPhenomD.IMRPhenomD_internals import PNPhasingSeriesTaylorF2
from IMRPhenomD.IMRPhenomD_internals import sigmaFits,betaFits,alphaFits
from IMRPhenomD.IMRPhenomD_internals import amp0Func,ComputeDeltasFromCollocation,gamma_funs,rho_funs
#from IMRPhenomD_internals import AmpIn

@njit()
def PhiInsPrefactorsMt(eta,Mt_sec,chis,chia,chi, _Lambda=0):
    """Helper function to get the prefactors for PhiIns"""
    v,vlogv = PNPhasingSeriesTaylorF2(eta,chis,chia,_Lambda)
    #  # PN phasing series
...
@njit()
def AmpPhaseSeriesInsAnsatz(Phi,dPhi,ddPhi,Amps,fs,Mt_sec,eta,chis,chia,chi,phi_ref,TTRef,amp_mult,NF_low,NF,_Lambda=0):
    ...
    prefactors_ini,prefactors_log = PhiInsPrefactorsMt(eta,Mt_sec,chis,chia,chi, _Lambda)
    rhos = rho_funs(eta,chi)
    amp_prefactors = AmpInsPrefactorsMt(Mt_sec,eta,chis,chia,rhos)
    amp0 = amp_mult/Mt_sec**(7/6)
...
@njit()
def PhiSeriesInsAnsatz(Phi,dPhi,ddPhi,fs,Mt_sec,eta,chis,chia,chi,phi_ref,TTRef,NF_low,NF,_Lambda=0):
...
def PhiSeriesIntAnsatz(Phi,dPhi,ddPhi,fs,Mt_sec,eta,chi,phi_ref,TTRef,NF_low,NF):
    ...
    if imrc.findT:
        ...
        Phi[NF_low:NF] = coeff1*np.log(Mt_sec) - phi_ref\
                            + (2*np.pi*TTRef+coeff0)*floc \
                            + coeff2/floc**3 \
                            + coeff1*np.log(floc) # is this where coalescence time tc gets plugged in???
...
@njit()
def IMRPhenDPhaseFI(Phis,times,timeps,fs,Mt_sec,eta,chis,chia,NF,MfRef_in,phi0, _Lambda=0):
    ...
    C1Int,C2Int,C1MRD,C2MRD = ComputeIMRPhenDPhaseConnectionCoefficients(fRD,fDM,eta,chis,chia,chi,MfMRDJoinPhi, _Lambda)
    ...
    if MfRef<imrc.PHI_fJoin_INS:
        TTRef =  -DPhiInsAnsatzInt(MfRef,eta,chis,chia,chi, _Lambda)
        phifRef = PhiInsAnsatzInt(MfRef,eta,chis,chia,chi, _Lambda)
    ...
    if itrfIntPhi>0:
        Phis,times,timeps = PhiSeriesInsAnsatz(Phis,times,timeps,fs,Mt_sec,eta,chis,chia,chi,phifRefIns,TTRefIns,0,itrfIntPhi, _Lambda) #Ins range
...
@njit()
def IMRPhenDAmpPhaseFI_get_TTRef(Mt_sec,eta,chis,chia,MfRef_in,imr_default_t=False,t_offset=0., _Lambda=0):
    ...
    # Compute coefficients to make phase C^1 continuous (phase and first derivative)
    _,C2Int,_,C2MRD = ComputeIMRPhenDPhaseConnectionCoefficients(fRD,fDM,eta,chis,chia,chi,MfMRDJoinPhi, _Lambda)
    ...
    if imr_default_t:
        ...
        if MfRef<imrc.PHI_fJoin_INS:
            dPhifRef =  -DPhiInsAnsatzInt(MfRef,eta,chis,chia,chi, _Lambda)
        elif MfRef<MfMRDJoinPhi:
...
@njit()
def IMRPhenDAmpPhaseFI(Phis,times,timeps,Amps,fs,Mt_sec,eta,chis,chia,NF,MfRef_in,phi0,amp_mult,imr_default_t=False,t_offset=0., _Lambda=0):
    ...
    Phis,times,timeps,Amps,itrFCut = IMRPhenDAmpPhase_tc(Phis,times,timeps,Amps,fs,Mt_sec,eta,chis,chia,NF,TTRef,phifRef,amp_mult, _Lambda)
    return Phis,times,timeps,Amps,TTRef,MfRef,itrFCut

@njit()
def IMRPhenDAmpPhase_tc(Phis,times,timeps,Amps,fs,Mt_sec,eta,chis,chia,NF,TTRef,phifRef,amp_mult, _Lambda=0):
    ...
    if itrfIntMax>0:
        Phis,times,timeps,Amps = AmpPhaseSeriesInsAnsatz(Phis,times,timeps,Amps,fs,Mt_sec,eta,chis,chia,chi,phifRefIns,TTRefIns,amp0,0,itrfIntMax, _Lambda) #Ins range
